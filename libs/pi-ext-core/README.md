...
This lib ships `.ts` sources and is designed for pi's [jiti](https://github.com/unjs/jiti) loader — no compile step required for pi consumers. It is not importable by plain Node without a loader. If a non-pi consumer ever needs it, add `tsup` and repoint `exports.require`.
