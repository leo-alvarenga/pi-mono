...
describe("update", () => {
  ...
  it("text > 120 chars → length error", () => {
    expect(
      applyAction(base(), { action: "update", id: 1, text: "x".repeat(121) })
        .ok,
    ).toBe(false);
  });
  ...
  it("blockedBy references unknown id → error", () => {
    expect(
      applyAction(base(), { action: "update", id: 1, blockedBy: [99] }).ok,
    ).toBe(false);
  });
  ...
  it("direct cycle A→B→A detected", () => {
    ...
    expect(applyAction(s, { action: "update", id: 2, blockedBy: [1] }).ok).toBe(
      false,
    );
  });
  it("indirect cycle A→B→C→A detected", () => {
    ...
    expect(applyAction(s, { action: "update", id: 3, blockedBy: [1] }).ok).toBe(
      false,
    );
  });
  it("status update with no other fields succeeds", () => {
    expect(
      applyAction(base(), { action: "update", id: 1, status: "completed" }).ok,
    ).toBe(true);
  });
});
