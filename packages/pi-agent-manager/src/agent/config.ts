...
import { AGENTS_DIR_NAME, AGENT_FILE_EXTENSION, AGENT_SHORTCUT_IDS } from "../constants";
...
export async function loadUserAgents(): Promise<LoadedUserAgents> {
  ...
  try {
    ...
    return { agents: [], errors: [`Failed to read agents dir: ${String(err)}`] };
  }
...
function validateAgent(
  ...
  // Name
  const name = (fm.name as string)?.toLowerCase() ?? basename(fileName, AGENT_FILE_EXTENSION).toLowerCase();
  ...
  if (type && !VALID_TYPES.includes(type)) {
    return { ok: false, error: `Invalid type "${type}" — must be primary or subagent` };
  }
...
function parsePermissions(raw: unknown): ParsedPerms {
  ...
  if (typeof raw === "string") {
    ...
    return { ok: false, error: `Unknown permission preset "${raw}". Valid: ${Object.keys(PERMISSION_PRESETS).join(", ")}` };
  }
  ...
  return { ok: false, error: "permissions must be a preset string, a legacy array, or a ruleset object" };
}
  ...
  for (const [permission, value] of Object.entries(obj)) {
    if (typeof value === "string") {
      if (!VALID_ACTIONS.includes(value as Action)) {
        return { ok: false, error: `Invalid action "${value}" for "${permission}" — must be allow, ask, or deny` };
      }
      ...
      for (const [pattern, action] of Object.entries(value as Record<string, unknown>)) {
        if (typeof action !== "string" || !VALID_ACTIONS.includes(action as Action)) {
          return { ok: false, error: `Invalid action "${String(action)}" for "${permission}" pattern "${pattern}"` };
        }
        ruleset.push({ permission, pattern, action: action as Action });
      }
    } else {
      return { ok: false, error: `Invalid value for "${permission}" — must be an action string or a pattern→action object` };
    }
  }
